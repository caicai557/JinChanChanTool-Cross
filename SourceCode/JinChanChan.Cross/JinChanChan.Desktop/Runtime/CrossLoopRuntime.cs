...
public sealed class CrossLoopRuntime : IDisposable
{
    ...
    private readonly Queue<string> _benchCardHistory = new();
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (shouldPurchase || shouldRefresh)
                {
                    ...
                    if (shouldPurchase)
                    {
                        UpdateBenchHistory(executePlan.PurchaseSlotIndexes, plan.RecognizedCards);
                    }
                ...
                if (_settings.EnableLineupAdvisor)
                {
                    LiveGameState gameState = _gameStateReader.Read(
                        ...
                        _benchCardHistory.ToArray(),
                        HoldRollPressed ? "roll" : "live");
    ...
    private void UpdateBenchHistory(
        IReadOnlyList<int> purchasedSlots,
        IReadOnlyList<string> recognizedCards)
    {
        foreach (int slot in purchasedSlots)
        {
            if (slot < 0 || slot >= recognizedCards.Count)
            {
                continue;
            }

            string hero = recognizedCards[slot];
            if (string.IsNullOrWhiteSpace(hero))
            {
                continue;
            }

            _benchCardHistory.Enqueue(hero.Trim());
        }

        while (_benchCardHistory.Count > 9)
        {
            _benchCardHistory.Dequeue();
        }
    }
}
