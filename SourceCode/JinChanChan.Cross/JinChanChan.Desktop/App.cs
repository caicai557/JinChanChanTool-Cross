...
public sealed partial class App : Application
{
    ...
    public override async void OnFrameworkInitializationCompleted()
    {
        if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            ...
            IOverlayPresenter overlayPresenter = new WindowsOverlayPresenter();
            ...
            IOverlayPresenter overlayPresenter = new MacOverlayPresenter();
            ...
            IReadOnlyList<LineupTemplateDefinition>? templates = null;
            if (string.Equals(settings.LineupDataSource, "local", StringComparison.OrdinalIgnoreCase))
            {
                string lineupPath = Path.Combine(resourcesPath, "Data", "lineup_templates.json");
                var (loadedTemplates, loadError) = await LineupTemplateStore.LoadAsync(lineupPath);
                if (loadError != null)
                {
                    report.Warnings.Add($"{loadError}，已回退内置模板。");
                }
                else
                {
            ...
            ILineupRecommendationService lineupRecommendation = new LineupRecommendationService(lineupMatcher, templates);
